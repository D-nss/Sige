<div class="">
    ...
    @if($user->hasAnyRole('super','edital-administrador') || ( $inscricao->atribuicao == 'Sub Comissão' && strtotime(date('Y-m-d')) >= strtotime($cronograma->getDate('dt_org_tematica', $inscricao->edital_id)) && strtotime(date('Y-m-d')) <= strtotime($cronograma->getDate('dt_termino_org_tematica', $inscricao->edital_id))))
        ...
        @endif
    @endif
    @if($user->hasAnyRole('super','edital-administrador') || ( $inscricao->atribuicao == 'Avaliação' && $inscricao->status == 'Deferido' && strtotime(date('Y-m-d')) >= strtotime($cronograma->getDate('dt_pareceristas', $inscricao->edital_id)) && strtotime(date('Y-m-d')) <= strtotime($cronograma->getDate('dt_termino_pareceristas', $inscricao->edital_id))))
