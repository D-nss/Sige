...
@foreach($indicadoresSerializado as $k => $indicadores)
    <li class="nav-item done"><a href="#step-{{ str_replace(".", "-", substr($k, 0, 4)) }}" class="nav-link">Item {{ substr($k, 0, 4) }} <i class="fal fa-comment-alt-lines" data-toggle="tooltip" data-placement="right" title="{{ $k }}"></i></a></li>
@endforeach
</ul>
