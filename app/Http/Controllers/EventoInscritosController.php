...
class EventoInscritosController extends Controller
{
    ...
    public function store(Request $request, Evento $evento)
    {
        ...
        $toValidate = [
            ...
            "tipo_documento"    => ($evento->ck_documento) ? 'required|max:190' : '',
            "documento"         => ($evento->ck_documento) ? 'required|max:190' : '',
            "sexo"              => ($evento->ck_sexo) ? 'required|max:190' : '',
            "genero"            => ($evento->ck_identidade_genero) ? 'required|max:190' : '',
            'instituicao'       => ($evento->ck_instituicao) ? 'required|max:190' : '',
            'pais'              => ($evento->ck_pais) ? 'required|max:190' : '',
            'area'              => ($evento->ck_area) ? 'required|max:190' : '',
            'vinculo'           => ($evento->ck_vinculo) ? 'required|max:190' : '',
            'deficiencia'       => ($evento->ck_deficiencia) ? 'required|max:4' : '',
            'desc_deficiencia'  => (isset($request->deficiencia) && $request->deficiencia == 'Sim') ? 'required|max:190' : '',
            'etnico_racial'     => ($evento->ck_racial) ? 'required|max:190' : '',
            'nascimento'        => ($evento->ck_nascimento) ? 'required|date|before:' . today() : '',
            'funcao'            => ($evento->ck_funcao) ? 'required|max:190' : '',
            'municipio'         => ($evento->ck_cidade_estado) ? 'required|max:190' : '',
            'personalizado'     => isset($evento->input_personalizado) ? 'required|max:190' : '',
        ];
    ...
    public function uploadArquivo(Request $request, $id)
    {
        ...
        if (isset($request->arquivo) || !$request->arquivo == '') {
            ...
            $arquivo = $upload->execute($request, 'arquivo', 'pdf', 5000000);
