...
class ComissaoAcaoController extends Controller
{
     /**
    ...
    public function store(Request $request)
    {
        ...
        $transacao = DB::transaction(function() use ($request, $user){
            ...
            if($comissao) {
                Log::channel('comissoes')->info('Usuario Nome: ' . Auth::user()->name . ' - Usuario ID: ' . Auth::user()->id . ' - Info: Comissao ID: '. $comissao->id .' criada com sucesso - Endereço IP: ' . $request->ip());
            }
            else{
                Log::channel('comissoes')->error('Usuario Nome: ' . Auth::user()->name . ' - Usuario ID: ' . Auth::user()->id . ' - Erro: Comissao NÃO criada - Endereço IP: ' . $request->ip());
            }
